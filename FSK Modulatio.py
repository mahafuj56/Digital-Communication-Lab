...
m = np.concatenate([A*np.cos(2*np.pi*(f1 if b else f2)*t) for b in x])
c1, c2 = np.cos(2*np.pi*f1*t), np.cos(2*np.pi*f2*t)

mn = [1 if (2*np.trapz(m[i:i+s]*c1, t)/bp) > A/2 else 0
